...
### 크레딧 예산 — 9월 1일까지 회당 상한 18

**2026-08-06 기준 잔액 80. 갱신분은 0이고 다음 갱신은 2026-09-01 이다.**
남은 월요일이 4번(8/10, 17, 24, 31)인데 이 80을 [[kpop_kit_factory]] 와 나눠 쓴다.
`Desktop/K-한류/kpop_kit_factory/COLLECT_VIDIQ.md` 가 같은 지갑을 본다. 여기서 태우면 거기가 못 돈다.

시작할 때 `vidiq_balance` 를 부른다(0 크레딧, 공짜). **잔액이 40 미만이면 추적만 하고 탐색은 통째로 건너뛴다.**

**9월 1일 전까지는 탐색 패스(1-B)를 하지 않는다.** 추적만 한다. 호출은 이 셋으로 끝낸다.

1. `vidiq_trending_videos` 영어권 1회 — 유튜브 키워드 전부를 이 한 번으로 재고, 새 후보도 눈에 띄면 여기서 줍는다
2. `vidiq_instagram_tiktok_outlier_search` 인스타 키워드 1회
3. 같은 도구로 틱톡 키워드 1회

새 키워드를 적극적으로 찾지 않는다. 지금은 **이미 추적 중인 것의 3주치를 채우는 게 훨씬 값어치 있다.** 신규 발굴은 9월 1일 갱신 후에 재개한다.

보고에 반드시 적는다: `vidiq 잔액 OO → OO (이번 실행 OO 사용)`
...
**초안을 쓰기 전에 마지막 발행일부터 확인한다.**

```bash
ls earlycatch-blog/published/ | sort | tail -1
```

파일명 앞의 날짜가 마지막 발행일이다. **오늘로부터 5일이 안 지났으면 초안을 쓰지 않는다.** 발행은 월요일 고정이라 정상 주기가 7일이고, 5일은 예외 상황(수동 발행 등)만 걸러내는 하한선이다. 데이터 갱신과 추적까지만 하고, 보고에 이렇게 적는다.

```
[블로그] 이번 주 건너뜀
  마지막 발행 2026-08-05 (1일 전) — 주 1회 주기 유지
```

**발행 요일은 월요일 고정이다.** 독자가 언제 올라오는지 아는 게 하루 이틀 간격보다 중요하다.

주 1회는 정한 약속이다. 쓸 거리가 있다고 이틀에 두 번 올리면 주기가 무너지고, 독자가 다음 글을 언제 기대할지 모르게 된다. 데이터는 매주 쌓되 발행은 주 1회다.

한 주 건너뛰면 그만큼 관측이 쌓여서 다음 글이 더 세진다. 2주치보다 3주치가 근거가 강하다.
